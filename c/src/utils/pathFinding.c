...
{
    // north
    if (y > 0 && cameFrom[y - 1][x][0] < 0 && checkTile(y-1 , x))
    {
    ...
    // south
    if (y < (MAX_HEIGHT - 1) && cameFrom[y + 1][x][0] < 0 && checkTile(y+1 , x))
    {
        addPositionYX(frontier, frontierCount, y + 1, x);
        frontierCount++;
        cameFrom[y + 1][x][0] = y;
    ...
    // east
    if (x < (MAX_WIDTH - 1) && cameFrom[y][x + 1][0] < 0 && checkTile(y , x + 1))
    {
    ...
    // west
    if (x > 0 && cameFrom[y][x - 1][0] < 0 && checkTile(y , x - 1))
    {
        addPositionYX(frontier, frontierCount, y, x - 1);
        frontierCount++;
        cameFrom[y][x - 1][0] = y;
...
void pathFind(Position * start, Position * end)
{
    ...
    while ((y != start->y || x != start->x) && (x!= -1 || y!= -1))
    {
            tmpY = y;
            y = cameFrom[tmpY][x][0];
            x = cameFrom[tmpY][x][1];
